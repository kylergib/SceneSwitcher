...
void MouseClick::Save(obs_data_t *obj, const char *name) const
{
	...
	_pos.Save(data);
	obs_data_set_int(data, "count", _count);
	_modifier.Save(data);
	obs_data_set_obj(obj, name, data);
	obs_data_release(data);
}
...
void MouseClick::Load(obs_data_t *obj, const char *name)
{
	...
	_pos.Load(data);
	_count = obs_data_get_int(data, "count");
	_modifier.Load(data);
	obs_data_release(data);
}
...
void PositionSelection::XChanged(int value)
{
	emit PositionChanged(Position{value, _y->value()});
}

void PositionSelection::YChanged(int value)
{
	  ...
	  _modifier(new ModifierSelection)
{
	...
	connect(_count, SIGNAL(valueChanged(int)), this,
...
void MouseClickSettings::CountChanged(int value)
{
	auto click = GetCurrentMouseClick();
	  ...
	  _modifier(new ModifierSelection)
{
	...
	++row;
	layout->addWidget(
		new QLabel(obs_module_text("AdvSceneSwitcher.key.pressCount")),
		row, 0);
	layout->addWidget(_count, row, 1);
	++row;
	layout->addWidget(
...
void KeyPressSettings::CountChanged(int value)
{
	auto key = GetCurrentKeyPress();
	key.SetCount(value);
	emit KeyPressSettingsChanged(key);
